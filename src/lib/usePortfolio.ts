...
const STORAGE_KEY = 'portfolio-tracker-stocks-v2';
...
export function usePortfolio() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (parsed.length > 0 && parsed[0].openPrice !== undefined) {
          setStocks(parsed);
        } else {
  ...
  const addStock = useCallback((stockData: {
    ...
    const newStock: Stock = {
      ...
      openPrice: stockData.currentPrice, // otevírací = aktuální při přidání
  ...
  // Výpočet statistik portfolia — denní změna na základě openPrice
  ...
  stats.totalProfitPercent = stats.totalInvested > 0
    ...
    : 0;
  stats.dayChangePercent = totalOpenValue > 0
    ? (stats.dayChange / totalOpenValue) * 100
    : 0;
